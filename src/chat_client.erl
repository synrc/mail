...
info(#'N2O'{tok=Key}=Msg, R, S) ->
   ...
   {reply, {default, Msg},R,S#cx{session = A}};
...
info(#'Ack'{lex=Key}, R,S) -> {reply, {text,<<"ACK ",(bin(Key))/binary>>},R,S};
